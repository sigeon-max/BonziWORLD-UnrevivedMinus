...
// --- BonziLog & Applets injection ---
try{
    window.clientcommands = window.clientcommands || {};
    clientcommands.bonzilog = function(){
        if(typeof newMsWindowOpened !== 'undefined' && newMsWindowOpened) return;
        new msWindow('BonziLog', `<div style="height:300px;overflow:auto;" id="bonzilog_content"><h3>BonziLog</h3><div id="bonzi_chatlog">No messages yet.</div><div id="applets_area" style="position:relative;margin-top:8px;"></div></div>`, undefined, undefined, undefined, undefined, [{name:'Close'}]);
        // add applets button area
        setTimeout(function(){
            var a = document.getElementById('applets_area');
            if(!a) return;
            var ap = document.createElement('div'); ap.className='applets_item';
            ap.innerHTML = '<img src="/img/icons/applets.png" width="40" height="auto" style="vertical-align:middle;margin-right:8px;"/><button id="open_applets" class="msBtn" style="max-height:40px;max-width:120px;">Applets</button>';
            a.appendChild(ap);
            document.getElementById('open_applets').onclick = function(){ if(clientcommands.applets) clientcommands.applets(); };
        },400);
    }

    clientcommands.applets = function(){
        new msWindow('Applets',`  
                                    <h1>BonziWORLD Applets</h1>
                                    <div id="appletsview">
                                            <div class="applets_item">
                                                <img src="/img/icons/notepad.png" width="50" height="auto"/>
                                            ...
                                            <div class="applets_item">
                                                ...
                                                <button id="bonzigay" class="msBtn"style="max-height:60px;max-width:100px;">Open BonziWORLD.int <h3>(New!)</h3></button>
                                    </div>
`, undefined, undefined, undefined, undefined, [ {name: "Close"} ]);

        setTimeout(() => {
            ["notepad","bonzigay"].forEach(applet => {
                var el = document.getElementById(applet);
                if(el) el.onclick = () => { clientcommands['applets_'+applet] && clientcommands['applets_'+applet](); }
            });
        },1100);
    }

    clientcommands['applets_notepad'] = () => {
        if(document.getElementById('notepadcont')) return;
        new msWindow('Notepad',`
                        ...
                        `, undefined, undefined, undefined, undefined, [{name: "Close"}]);
        setTimeout(() => {
            document.getElementById('notepadcopy').onclick = function(){
                var ta = document.getElementById('notepadcont');
                if(ta.selectionStart === ta.selectionEnd){alert('No text selected. Hold and drag to select text.');return;} else { clipboard(ta.value.substring(ta.selectionStart,ta.selectionEnd)); }
            };
            document.getElementById('notepadpaste').onclick = function(){ getClipboard().then(clipcont => { var ta = document.getElementById('notepadcont'); var result = ta.value.substring(0, ta.selectionStart) + clipcont + ta.value.substring(ta.selectionStart, ta.value.length); ta.value = result; }); };
            document.getElementById('notepadrun').onclick = function(){ try{ eval(document.getElementById('notepadcont').value); } catch(e){ alert(e); } };
        },1100);
    };

    clientcommands['applets_bonzigay'] = () => {
        if(document.getElementById('erik')) return;
        new msWindow('BonziWORLD.int',`<iframe src="https://bonziworld.qzz.io/" width="600" height="450" id="erik"></iframe>`,undefined,undefined,undefined,undefined,[]);
    };
}catch(e){}
// small helpers and theme CSS for BonziLog/Applets
try{
    ...
    (function(){
        if(document.getElementById('bonzilog_theme_css')) return;
        var css = '\n#bonzilog_content{font-family:Tahoma,Segoe UI,Arial,sans-serif;background:linear-gradient(180deg,#2b0f4d,#1b0830);color:#fff;padding:8px;border-radius:6px;}\n#bonzi_chatlog{max-height:220px;overflow:auto;background:rgba(255,255,255,0.04);padding:8px;border-radius:4px;}\n.bonzi_log_entry{padding:6px 4px;border-bottom:1px solid rgba(255,255,255,0.03);}\n.bonzi_log_entry:last-child{border-bottom:0;}\n.bl_ts{color:#d6b3ff;margin-right:6px;font-size:11px;}\n.bl_name{color:#ffffff;font-weight:700;margin-right:6px;}\n.bl_msg{color:#f3eefe;}\n#appletsview{background:linear-gradient(180deg,#3a0f5a,#240735);padding:12px;border-radius:6px;color:#fff;}\n.applets_item{display:inline-block;margin:8px;padding:6px;background:rgba(255,255,255,0.03);border-radius:6px;text-align:center;}\n.applets_item img{display:block;margin:0 auto 6px;}\n.msBtn{background:#8f5ce6;color:#fff;border:0;padding:6px 8px;border-radius:4px;cursor:pointer;}\n.msBtn:hover{background:#a576f5;}\n';
        var s = document.createElement('style'); s.id='bonzilog_theme_css'; s.appendChild(document.createTextNode(css)); document.head.appendChild(s);
    })();
}catch(e){}
// delegated click handler to ensure UI buttons inside dynamically-created windows trigger commands
try{
    document.addEventListener('click', function(ev){
        var t = ev.target;
        // normalize if click on img inside button
        if(t && t.tagName === 'IMG' && t.parentElement && t.parentElement.id) t = t.parentElement;
        if(!t || !t.id) return;
        if(t.id === 'open_applets' || t.id === 'applets_button'){
            if(window.clientcommands && clientcommands.applets) clientcommands.applets();
        }
        if(t.id === 'bonzilog_button'){
            if(window.clientcommands && clientcommands.bonzilog) clientcommands.bonzilog();
        }
        if(t.id === 'notepad'){
            if(window.clientcommands && clientcommands['applets_notepad']) clientcommands['applets_notepad']();
        }
        if(t.id === 'bonzigay'){
            if(window.clientcommands && clientcommands['applets_bonzigay']) clientcommands['applets_bonzigay']();
        }
    }, false);
}catch(e){}
// ensure titlebar buttons exist for BonziLog/Applets
try{
    (function(){
        function ensureButtons(){
            var tb = document.getElementById('titlebar');
            if(!tb) return setTimeout(ensureButtons,200);
            if(document.getElementById('bonzilog_button')) return;
            var wrap = document.createElement('div');
            wrap.id = 'bonzilog_header_buttons';
            wrap.style.position = 'absolute'; wrap.style.right='8px'; wrap.style.top='8px'; wrap.style.display='flex'; wrap.style.gap='8px'; wrap.style.alignItems='center'; wrap.style.zIndex='9999'; wrap.style.pointerEvents='auto';
            var b = document.createElement('div'); b.id='bonzilog_button'; b.style.cursor='pointer'; b.style.pointerEvents='auto'; b.innerHTML = '<img src="./img/icons/log_button.webp" alt="BonziLog" width="36" height="36"/>';
            var a = document.createElement('div'); a.id='applets_button'; a.style.cursor='pointer'; a.style.pointerEvents='auto'; a.innerHTML = '<img src="./img/icons/applets.png" alt="Applets" width="36" height="36"/>';
            wrap.appendChild(b); wrap.appendChild(a); tb.appendChild(wrap);
            b.addEventListener('click', function(ev){ ev.stopPropagation(); if(window.clientcommands && clientcommands.bonzilog) clientcommands.bonzilog(); });
            a.addEventListener('click', function(ev){ ev.stopPropagation(); if(window.clientcommands && clientcommands.applets) clientcommands.applets(); });
        }
        ensureButtons();
    })();
}catch(e){}
    ...
    loadQueue.loadManifest([
       { id: "bonziAqua", src: "./img/bonzi/aqua.png" },
        ...
        { id: "bonziPurplesaber", src: "./img/bonzi/purplesaber.png" },
                ...
                { id: "bonziLightcyancat", src: "./img/bonzi/lightcyancat.png" },
                { id: "bonziRedblackguy", src: "./img/bonzi/redblackguy.png" },
        { id: "bonziKrosh", src: "./img/bonzi/krosh.png" },
        { id: "bonziMiracle-machine", src: "./img/bonzi/miracle-machine.png" },
        ...
        { id: "bonziLavenderribbon", src: "./img/bonzi/lavenderribbon.png" },
                { id: "bonziClock", src: "./img/bonzi/clock.png" },
                { id: "bonziPurpke", src: "./img/bonzi/purpke.png" },
                { id: "bonziMarcello", src: "./img/bonzi/marcello.png" },
                { id: "bonziDistorted", src: "./img/bonzi/distorted.png" },
                { id: "bonziRaging", src: "./img/bonzi/raging.png" },
                { id: "bonziNewbie", src: "./img/bonzi/newbie.png" },
                { id: "bonziFloyd", src: "./img/bonzi/floyd.png" },
                { id: "bonziEggy", src: "./img/bonzi/eggy.png" },
                { id: "bonziBlackf", src: "./img/bonzi/blackf.png" },
                { id: "bonziMonke", src: "./img/bonzi/monke.png" },
                { id: "bonziGeogd", src: "./img/bonzi/geogd.png" },
                { id: "bonziTable", src: "./img/bonzi/table.png" },
                { id: "bonzi27", src: "./img/bonzi/27.png" },
                { id: "bonziEkrot", src: "./img/bonzi/ekrot.png" },
                { id: "bonziLaxy", src: "./img/bonzi/laxy.png" },
                { id: "bonziLosky", src: "./img/bonzi/losky.png" },
                { id: "bonziMerlin", src: "./img/bonzi/merlin.png" },
                { id: "bonziMilu", src: "./img/bonzi/milu.png" },
                { id: "bonziYellowy", src: "./img/bonzi/yellowy.png" },
      ...
      function setup() {
        ...
        socket.on("update", function (a) {
            ...
            socket.on("talk", function (a) {
                ...
                try{
                    var logEl = document.getElementById('bonzi_chatlog');
                    if(logEl){
                        var name = (window.usersPublic && usersPublic[a.guid] && usersPublic[a.guid].name) ? usersPublic[a.guid].name : (a.name || a.guid || 'Unknown');
                        var entry = document.createElement('div');
                        entry.className = 'bonzi_log_entry';
                        var ts = new Date();
                        ...
                        entry.innerHTML = '<span class="bl_ts">['+timestr+']</span> <span class="bl_name">'+escapeHtml(name)+':</span> <span class="bl_msg">'+linkify(escapeHtml(a.text || ''))+'</span>';
       ...
       socket.on("video", function (a) {
        ...
        socket.on("unvaporwave", function (a) {
            ...
            var startX = b.x, startY = b.y;
            var sz = (b.data && b.data.size) ? b.data.size : { x: 200, y: 160 };
            try {
                var sfx = new Audio("./explosion.mp3");
                sfx.volume = 0.7;
                sfx.play().catch(function () {});
                var $ex = $('<div class="explosion"></div>').css({
                    left: (startX + sz.x / 2) + "px",
                    top:  (startY + sz.y / 2) + "px"
                });
                $("body").append($ex);
                setTimeout(function () { $ex.remove(); }, 900);
            } catch (e) {}
            // Pivot rotation around sprite center
            b.sprite.regX = sz.x / 2;
            b.sprite.regY = sz.y / 2;
            var rot = 0, dx = 0, dy = 0;
            var angvel = (Math.random() * 30 + 20) * (Math.random() > 0.5 ? 1 : -1);
            var xvel = (Math.random() * 10 + 5) * (Math.random() > 0.5 ? 1 : -1);
            var yvel = -20, i = 0;
            var iv = setInterval(function () {
                i++; yvel += 2; dx += xvel; dy += yvel; rot += angvel;
                b.sprite.x = startX + dx + sz.x / 2;
                b.sprite.y = startY + dy + sz.y / 2;
...
var _createClass = (function () {
                        ...
                        return {
                            items: {
                                ...
                                kiddie: {
                                    name: "Call an Kiddie",
                                    callback: function () {
                                        socket.emit("command", { list: ["kiddie", d.userPublic.name] });
                                    },
                                },
                                logofag: {
                                    name: "Call an Logofog",
                                    callback: function () {
                                ...
                                fun: {
                                    ...
                                    items: {
                                        ...
                                        nameedit: {
                                            name: "Change Name",
                                            callback: function () {
                                                socket.emit("command", { list: ["nameedit", d.id, prompt("give this guy a name")] });
                                            },
                                        },
                                        nuke: {
                                            name: "NUKE",
    ...
    BonziData = {
        ...
        event_list_joke_open: [
            ...
            [{ type: "text", text: "Ready for some classic Jewish comedy?" }],
            [
        ...
        event_list_joke_mid: [
            ...
            [
                ...
                { type: "text", text: "Sonic's is pink! HAHAHA- ACK!." },
                { type: "anim", anim: "shrug_back", ticks: 15 },
            ],
            [
            ...
            [
                ...
                { type: "text", text: "Because we KNEAD it. Haha please send money to my PayPal at NAH NOT THE EMAIL LEAKER@bonzi.com" },
                { type: "anim", anim: "shrug_back", ticks: 15 },
            ],
            [
...
var loadQueue = new createjs.LoadQueue(),
    loadDone = [],
    loadNeeded =  ["bonziAqua", "bonziBlack", "bonziBlue", "bonziBrown", "bonziCyan", "bonziDiamond", "bonziEmerald", "bonziGold", "bonziGreen", "bonziLime", "bonziOrange", "bonziPink", "bonziPurple", "bonziRed", "bonziPink", "bonziWhite", "bonziYellow",  "bonziPurplesaber", "bonziKrosh", "bonziMiracle-machine" ,"bonziNethergold","bonziQuartz"  ,"bonziCartoonnetwork", "bonziBrasilempire","bonziStella","bonziGrinnyboi","bonziYan",  "bonziPeedy",  "bonziBustystickwoman", "bonziRuby", "bonziNavy", "bonziSapphire", "bonziChartreuse", "bonziLavenderribbon", "bonziFemboykisser", "bonziClock",  "bonziJew", "bonzuJabba", "bonziRonnie", "bonziLightcyancat", "bonziRedblackguy","topjej"];
...
$(function () {
    ...
    socket.on("kick", function (a) {
      ...
      (audio = new Audio("./sfx/louddoggis.mp3")),
        audio.play(),
        socket.emit("command", { list: ["name", "Doggis"] });
